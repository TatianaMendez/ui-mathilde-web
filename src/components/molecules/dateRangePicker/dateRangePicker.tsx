...
export const DateRangePicker: React.FC<DateRangePickerProps> = ({
  ...
  const [internalRange, setInternalRange] = useState<RangeKey[]>([
    {
      startDate: new Date(),
  ...
  const quickRangesToUse = quickRanges ?? defaultQuickRanges;

  // Si value está definida, úsala; si no, usa el estado interno
  const range = value
    ? [{ ...value, key: 'selection' }]
    : internalRange;
  ...
  const handleRangeChange = (item: { selection: RangeKey }) => {
    if (!value) setInternalRange([item.selection]);
  ...
  const handleQuickRange = (rangeFn: () => { startDate: Date, endDate: Date }) => {
    ...
    if (!value) setInternalRange([{ startDate, endDate, key: 'selection' }]);
